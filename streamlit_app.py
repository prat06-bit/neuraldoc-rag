...
# ═════════════════════════════════════════════════════════════════════════════
# CHAT PAGE — Vertical layout: Upload first, then Chat
# ═════════════════════════════════════════════════════════════════════════════
else:
    st.html("""<style>
    ...
    def get_health():
        ...
        except: return {"pipeline_ready":False,"total_chunks":0,"indexed_files":[]}
    ...
    # ── Fix: provide label and hide it to avoid warning ───────────────────────
    ...
    if h_check.get("indexed_files"):
        st.html("""<div style="display:flex;flex-wrap:wrap;gap:8px;margin:12px 0;">""")
    ...
    # Query input — provide proper label, hide it via label_visibility
    ...
    if ask and query:
        if not h_check.get("pipeline_ready"):
            st.warning("Upload and index a PDF first, then ask questions.")
        else:
